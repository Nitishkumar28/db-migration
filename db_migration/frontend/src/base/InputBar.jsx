...
const InputBar = ({ title, db_type, field, type = "text" }) => {
  ...
  const activeConnectionDetails = useDBStore(state => {
    const current = state.connectionDetails.find(conn => conn.db_type === activeConnection?.toLowerCase())
      return current;
      /*
    if(db_type === "source") {
  ...
  const handleChange = (e) => {
    updateConnectionDetails(activeConnection?.toLowerCase(), field, e.target.value);
  }
  ...
  return (
    <div className={`flex flex-col justify-start items-start`}>
      ...
      <input
        ...
        className={`w-full outline-none border border-gray-400 rounded px-2 py-1 ${!activeConnection && "cursor-default bg-gray-100 pointer-events-none"}`}
      />
    </div>
  );
};
